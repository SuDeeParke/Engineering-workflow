# Engineering Lifecycle Plugin

This folder is a generated local Codex plugin. It packages the rebuilt skills from output/skills into a plugin layout under output/plugin/engineering-lifecycle.
...
Plugin-level hooks use 
ode so the same hook commands can run on macOS, Linux, and Windows without depending on ash or pwsh.

Run the self-test from the plugin root or repository root with:

`ash
node output/plugin/engineering-lifecycle/scripts/test-plugin-hooks.mjs
`
